...
let httpUrl = 'http://111.231.75.86:8000/api/provinces/USA/daily/'
...
function reqdailyUSAAll() {
    axios.get(httpUrl).then((res) => {
        if (res.data.length !== 0) {
            queryHandle1('TRUNCATE TABLE dailyUSA')
            for (let i = 0; i < res.data.length; i++) {
                ...
                let strSql = 'insert into dailyUSA (dateId,provinceCode,provinceName,countryCode,confirmedCount,currentConfirmedCount,suspectedCount,curedCount,deadCount,currentConfirmedIncr,confirmedIncr,suspectedIncr,curedIncr,deadIncr) value (?,?,?,?,?,?,?,?,?,?,?,?,?,?)'
